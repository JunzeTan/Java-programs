...
class Database {
    ...
    int numCustomersOfPlan(String planName){
    ...
    /* get the total paid amount to the customers for a specific plan */
    long totalPaidToCustomer(String planName) {
        long totalPaidToCusotmer = 0;
        if (planName != null) {
            for (Plan plan : plans) {
                if (plan.getName().equals(planName)) {
                    for (Contract contract : contracts) {
                        if (contract.getPlanName().equals(planName)) {
                            ...
                            totalPaidToCusotmer += this.totalReceivedAmountByCustomer(customerName);
                        }
                    }
                }
            }
        }
        return totalPaidToCusotmer;
    }
}
